  ...
  (defun directory-files (directory &optional (pattern *wild-file*))
    (let ((dir (pathname directory)))
      ...
      (let* ((pat (merge-pathnames* pattern dir))
             ...
             ;; Since we are not following links, Clozure CL will report links
             ;; to directories using file pathnames.
             #+clozure
             (entries (mapcar #'(lambda (f)
                                  (if (directory-exists-p f)
  ...
  (defun subdirectories (directory)
    (let* ((directory (ensure-directory-pathname directory))
           ...
           (wild (merge-pathnames*
                  #-(or abcl allegro clozure cmu lispworks sbcl scl xcl)
                  *wild-directory*
                  #+clozure
                  *wild-file*
           ...
           (dirs
             #-(or abcl cormanlisp genera xcl)
             (ignore-errors
              (directory* wild . #.(or #+clozure '(:directories t :files t)
                                       #+mcl '(:directories t))))
             #+(or abcl xcl) (system:list-directory directory)
             #+cormanlisp (cl::directory-subdirs directory)
             #+genera (fs:directory-list directory))
           #+(or abcl allegro clozure cmu genera lispworks sbcl scl xcl)
           (dirs (loop :for x :in dirs
                       :for d = #+(or abcl xcl) (extensions:probe-directory x)
                       #+allegro (excl:probe-directory x)
                       #+clozure (directory-exists-p x)
                       #+(or cmu sbcl scl) (directory-pathname-p x)
                       #+genera (getf (cdr x) :directory)
                       #+lispworks (lw:file-directory-p x)
                       :when d :collect #+(or abcl allegro xcl) d
                         #+clozure (ensure-directory-pathname x)
